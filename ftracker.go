...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Плавание":
		distance := distance(action)                                               // вызовите здесь необходимую функцию
		speed := swimmingMeanSpeed(action, countPool, duration)                    // вызовите здесь необходимую функцию
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return ((runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift) * weight / duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	meanSpeedPow := (math.Pow(meanSpeed(action, duration), 2) / height)
	return (walkingCaloriesWeightMultiplier*weight + meanSpeedPow*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	swimMeanSpeed := swimmingMeanSpeed(lengthPool, countPool, duration)
	return (swimMeanSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
